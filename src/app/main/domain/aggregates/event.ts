...
import {
  createEventCreated,
  createEventPublished,
  createEventCancelled,
  createTicketCategoryCreated,
  createTicketCategoryDisabled,
...
export const publishEvent = (event: EventState): void => {
  ...
  if (getTotalQuota(event) > event.maxCapacity)
...
export const addCategory = (
  ...
  if (event.status !== "draft" && event.status !== "published")
    throw new Error("Cannot add category to this event");
  if (salesEnd > event.startDate)
    throw new Error("Sales period must end before or at the event start date");
  ...
  if (getTotalQuota(event) + quota > event.maxCapacity)
    throw new Error("Total ticket quota cannot exceed event capacity");
...
export const disableCategory = (
  ...
  if (event.status === "completed")
    throw new Error("Cannot disable category on completed event");
...
export const reserveCategory = (
  ...
  if (getSalesStatus(category) !== "active")
    throw new Error("Ticket category is not available for sale");
