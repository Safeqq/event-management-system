...
import {
  createRefundRequested,
  createRefundApproved,
  createRefundRejected,
  createRefundPaidOut,
...
export const rejectRefund = (refund: RefundState, reason: string): void => {
  ...
  addDomainEvent(
    refund,
    createRefundRejected(refund.id, refund.bookingId, reason),
  );
};

export const payoutRefund = (
  refund: RefundState,
  ...
  addDomainEvent(
    refund,
    createRefundPaidOut(refund.id, refund.bookingId, paymentReference),
  );
};
